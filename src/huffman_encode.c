...
static void traverse_tree(const int depth, const Node *np)
{
    ...
    {
        for (int i=0; i<strlen(np->huffman_code); ++i)
            printf("    ");
        /*
        表示のところめちゃくちゃ頑張ったんですが、どうしてもうまく行かなくて、
        締め切りまで時間もうないのでコードを全部消して、
        一番シンプルなやり方で表示しました．（二分木の雰囲気だけ）
        テスト終わってから課題やろうと思ったら時間が足りなくなってしまって申し訳ございません．
        */
        if (np->symbol == (int)'\n')
            printf("EOL %s\n", np->huffman_code);
